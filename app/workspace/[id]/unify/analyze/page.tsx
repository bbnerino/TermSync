...
type AnalysisStep = 'parsing' | 'extracting' | 'grouping' | 'matching' | 'completed';
...
export default function UnifyAnalyzePage() {
  ...
  const { files, setParsedDocuments, setTermGroups, setStep, setLoading } = useUnifyStore();
  ...
  async function startAnalysis() {
    try {
      ...
      const parseResponse = await fetch('/api/unify/parse', {
        method: 'POST',
        body: formData,
      });
  ...
  const steps = [
    { key: 'parsing', label: '문서 파싱', description: '문서 구조와 내용을 분석하고 있습니다.', icon: 'sync' },
    { key: 'extracting', label: '용어 추출', description: '문서에서 주요 용어를 식별하고 있습니다.', icon: 'manage_search' },
    { key: 'grouping', label: '용어 그룹화', description: '유사한 용어들을 그룹화하고 있습니다.', icon: 'workspaces' },
    { key: 'matching', label: 'DB 매칭 및 AI 분석', description: '기존 DB와 매칭하고 AI로 표준화하고 있습니다.', icon: 'psychology' },
  ];
            ...
            <div className="flex flex-col gap-3 max-w-2xl">
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary border border-primary/20">
                  {error ? '실패' : currentStep === 'completed' ? '완료' : '처리 중'}
                </span>
              ...
              <h1 className="text-3xl font-black leading-tight tracking-[-0.033em] text-white md:text-4xl">
                {error ? '분석 실패' : currentStep === 'completed' ? '분석 완료!' : 'AI 분석 진행 중'}
              </h1>
              ...
              <div className="flex flex-col gap-4 rounded-xl border border-surface-border bg-surface-dark p-6 shadow-md">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-base font-medium text-white">전체 진행률</p>
                      <p className="text-sm text-text-dim">
                        {error ? '분석 실패' : steps[currentStepIndex]?.label || '처리 중...'}
                      </p>
                    </div>
                  </div>
                  <p className={`text-2xl font-bold ${error ? 'text-red-400' : 'text-primary'}`}>
                    {error ? '오류' : `${progress}%`}
                  </p>
                </div>
                <div className="relative h-3 w-full overflow-hidden rounded-full bg-[#1a2e23]">
                  <div 
                    className={`absolute left-0 top-0 h-full rounded-full transition-all duration-500 ease-out ${
                      error ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-primary shadow-[0_0_10px_rgba(43,238,121,0.5)]'
                    }`}
                      ...
                      <div key={step.key} className="contents">
                        ...
                        <div className="flex flex-col items-center">
                          <div className={`relative flex h-8 w-8 items-center justify-center rounded-full ${
                            ...
                            <span className={`material-symbols-outlined text-[18px] ${isActive ? 'animate-spin' : ''}`}>
                              {isCompleted ? 'check' : step.icon}
                            </span>
                          </div>
                          {index < steps.length - 1 && (
                            <div className={`h-full w-[2px] my-2 ${
                              isCompleted
                                ? 'bg-primary'
                                : isActive
                        ...
                        {/* Content */}
                        <div className={`pb-8 pt-1 ${isPending ? 'opacity-60' : ''}`}>
                          <p className="text-base font-bold text-white">{step.label}</p>
                          <p className={`text-sm mt-1 font-medium ${
                            isActive ? 'text-primary' : isCompleted ? 'text-green-400' : 'text-text-dim'
                          }`}>
                            {isCompleted ? '완료' : isActive ? '처리 중...' : '대기 중'}
                          </p>
                          <p className="text-xs text-text-dim mt-1">{step.description}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="flex flex-col gap-6 lg:col-span-5">
              ...
              <div className="rounded-xl border border-surface-border bg-surface-dark p-6 shadow-md">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-white">분석 중인 문서</h3>
                </div>
                <div className="flex flex-col gap-3">
                  {files.map((file, index) => (
                    <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-[#1a2e23] border border-surface-border">
                      <span className="material-symbols-outlined text-text-dim text-[20px]">article</span>
              ...
              <div className="rounded-xl border border-yellow-900/30 bg-yellow-900/10 p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-white mb-1">분석 시간</p>
                    <p className="text-xs text-text-dim leading-relaxed">
                      문서 크기와 복잡도에 따라 1-3분 정도 소요됩니다. 
                      분석이 완료되면 자동으로 검토 화면으로 이동합니다.
                    </p>
                  </div>
                </div>
              </div>
