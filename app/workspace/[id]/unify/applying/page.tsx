...
export default function UnifyApplyingPage() {
  ...
  const { parsedDocuments, termGroups, saveToDb, setLoading, setError, reset } =
    useUnifyStore();
  ...
  const totalChanges = selectedGroups.reduce(
    (sum, g) => sum + g.occurrences.filter((o) => o.selected).length,
    0
  );
  ...
  async function applyTermUnification() {
    try {
      ...
      for (let i = 0; i < parsedDocuments.length; i++) {
        ...
        for (let j = 0; j < docChanges.length; j++) {
          ...
          if (j === 0 || j === docChanges.length - 1 || j % 3 === 0) {
            addLog(
              `${doc.name} - ${occ.line}행: "${occ.before}" → "${occ.after}"`
            );
          }
      ...
      if (saveToDb) {
        addLog(
          `✅ ${parsedDocuments.length}개 문서가 Storm DB에 저장되었습니다`
        );
        addLog(
          `✅ ${selectedGroups.length}개 용어 매핑이 "${currentWorkspace?.name}" DB에 저장되었습니다`
        );
      }
  ...
  const getStatusIcon = (docIndex: number) => {
    ...
    if (docIndex < currentDoc)
      return { icon: "check_circle", color: "text-primary" };
    if (docIndex === currentDoc)
      return { icon: "sync", color: "text-yellow-400 animate-spin" };
          ...
          <div className="rounded-xl border border-border-green bg-surface-dark p-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  status === "saving" ||
                  (status === "applying" &&
                    currentDoc === parsedDocuments.length - 1)
                    ? "check_circle"
                    : status === "parsing" || status === "applying"
                    ? "sync"
                    : "schedule"}
                ...
                >
                  ...
                  {status === "done" || status === "saving"
                    ...
                    : "중"}
                  {(status === "parsing" || status === "applying") &&
                    ` (${currentDoc + 1}/${parsedDocuments.length})`}
                </span>
              </div>
            ...
            {status !== "done" && (
              <div className="mt-4 text-text-dim text-sm">
                예상 소요 시간: 약{" "}
                {Math.max(1, Math.ceil((100 - progress) / 10))}초
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-border-green bg-surface-dark p-6">
            ...
            <div className="flex flex-col gap-2">
              {parsedDocuments.map((doc, index) => {
                ...
                const docChanges = selectedGroups.flatMap((g) =>
                  g.occurrences.filter(
                    (o) => o.selected && o.docName === doc.name
                  )
                ).length;
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-text-dim text-sm">
                      {index < currentDoc || status === "done"
                        ? "완료"
                        : index === currentDoc && status === "applying"
                        ? "처리 중..."
                        : "대기 중"}{" "}
                      ({docChanges}건)
                    </span>
